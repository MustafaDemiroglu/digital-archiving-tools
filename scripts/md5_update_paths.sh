#!/usr/bin/env bash
###############################################################################
# Script Name: md5_update_paths.sh (Version 2.3)
#
# Description:
...
# Process simple rename CSV (full_path_old,full_path_new) - FIXED
...
# Process all MD5 files in directory - UPDATED with CSV type selection
...
# Show main menu - UPDATED descriptions
show_main_menu() {
...
output_and_log "${GREEN}=== Script Execution Completed Successfully! ===${NC}"\n'
            ...
            if [ "$found_match" = true ]; then
                ...
                [ "$VERBOSE" = true ] && echo "$MD5_CONTENT" | grep -o '[[:space:]].*
...
#output_and_log ""
#output_and_log "${GREEN}=== Script Execution Completed Successfully! ===${NC}" | head -5
#           fi
#        done
#    } < "$csv_file"
